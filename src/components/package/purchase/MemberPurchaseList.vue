<template>
     <v-card>
        <div class="d-flex align-baseline flex-row header">
          <div>
            <h2 class="header-title">Member Purchase Table</h2>
            <v-select
              :items="members"
              class="ml-1"
              item-text="name"
...
export default {
    ...
    computed:{
      ...
      ...mapState('PackagePurchase',['memberPurchases'])
    },
    data() {
      return {
        dialog:false,
        ...
        headers: [
          ...
          { text: "Package Name", value: "member_package.name",},
          { text: "Day Spend", value: "days_left.day_spend",},
          { text: "Duration", value: "days_left.duration",},
          { text: "Price", value: "member_package.price",},
  ...
  methods:{
    getPurchaseByMemberId(memberId){
      this.$store.dispatch('PackagePurchase/getPurPackagesByMember',memberId)
    },
    showImage(purchase){
  ...
  created(){
    this.$store.dispatch('Member/getMembers')
     ...
     this.$store.dispatch('PackagePurchase/getPurPackagesByMembers')
  }
}
</script>
