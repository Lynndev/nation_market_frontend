...
<v-card>
    ...
    <v-data-table :headers="headers" :loading="loading" @click="seeMember(member)" :search="search" :items="members" :items-per-page="10">
        <template v-slot:item.actions="{ item }">
            <v-icon color="yellow darken-2" class="mr-2" @click="editMember(item)">
                mdi-pencil-outline
            </v-icon>
            <v-icon color="blue darken-2" class="mr-2" @click="chatMember(item)">
                mdi-message-outline
            </v-icon>
        </template>
    </v-data-table>
