...
const actions = {
  storeProperty({ dispatch }, payload) {
    Property.storeProperty(payload).then((response) => {
  ...
  updateProperty({ dispatch }, payload) {
    Property.updateProperty(payload).then((response) => {
      console.log(response);
      const notification = {
        type: "success",
        status: true,
  ...
  deleteProperty({ dispatch }, payload) {
    Property.deleteProperty({ id: payload.id }).then((response) => {
      console.log(response);
  ...
  getPropertiesByCatId({ commit }, catId) {
    Property.getPropertiesByCatId(catId).then((properties) => {
