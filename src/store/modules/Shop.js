...
const actions = {
  storeShop({ dispatch }, payload) {
    Shop.storeShop(payload).then((response) => {
  ...
  changeStatus({ dispatch }, payload) {
    console.log(payload);
    Shop.changeStatus(payload).then((response) => {
      console.log(response);
  ...
  getEachShop({ commit }, payload) {
    Shop.getEachShop(payload).then((shop) => {
      console.log(shop);
      commit("SET_EACH_SHOP", shop);
    });
  },
  getShopByMainCatId({ commit }, mainCatId) {
    Shop.getShopByMainCatId(mainCatId).then((shops) => {
      commit("SET_SHOPS", shops);
      console.log(shops);
    });
  },
  getShops({ commit }) {
    Shop.getShops().then((shops) => {
      commit("SET_SHOPS", shops);
      console.log(shops);
    });
  },
