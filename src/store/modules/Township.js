...
const actions = {
  storeTownship({ dispatch }, payload) {
    Township.storeTownship(payload).then(() => {
      const notification = {
        type: "success",
        status: true,
  ...
  updateTownship({ dispatch }, payload) {
    Township.updateTownship(payload).then(() => {
      const notification = {
        type: "success",
        status: true,
  ...
  deleteTownship({ dispatch }, payload) {
    Township.deleteTownship({ id: payload.id }).then((response) => {
  ...
  getTownshipByStateId({ commit }, stateId) {
    Township.getTownshipByStateId(stateId).then((townships) => {
